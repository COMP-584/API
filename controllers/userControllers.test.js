...
const testUser = { name: "test", email: "test@gmail.com", password: "123456"};
let responseUser;
...
test("Should search for other users!", async () => {
  ...
  await request(app).get("/api/user?search=Monica")
    .set("Authorization", `Bearer ${response.body.token}`).expect(200);
});
